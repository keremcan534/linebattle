...
export function generateProvinces(
  ...
  floodFill(raw, seeds, width, height, passable);
  ...
  for (let i = 0; i < n; i++) {
    if (raw[i] === NO_PROVINCE && passable(i)) {
      ...
      seeds.push(i);
      raw[i] = id;
      floodFill(raw, [i], width, height, passable, id);
    }
  }

  return build(raw, seeds.length, terrain, alliances);
}
...
function floodFill(
  ...
  while (queue.length) {
    for (const i of queue) {
      ...
      const x = i % width;
      const y = (i / width) | 0;
      if (x > 0) tryClaim(i - 1, pid, cell, passable, next);
      if (x < width - 1) tryClaim(i + 1, pid, cell, passable, next);
      if (y > 0) tryClaim(i - width, pid, cell, passable, next);
      if (y < height - 1) tryClaim(i + width, pid, cell, passable, next);
    }
    queue = next;
    next = [];
  }
...
function tryClaim(
  ...
  if (cell[j] !== NO_PROVINCE || !passable(j)) return;
  cell[j] = pid;
  next.push(j);
}
...
function build(
  ...
  return new ProvinceMap(provinces, cellProvince, terrain, alliances);
}
